function A = Approx5Obj(frente,epsilon)
% HV Approximation calculation using the model_15
...
try
    econ = bestTraining_15(con_ind);
catch
    uiwait(warndlg('Problem using model. This model is trained for five objectives.'));
end
%GP Model without individual
esin = bestTraining_15(sin_ind);
A = zeros(size(esin));
for i = 1 : n
