function A = Approx4Obj(frente,epsilon)
% HV Approximation calculation using the model_56
...
try
    econ = bestTraining_56(con_ind);
catch
    uiwait(warndlg('Problem using model. This model is trained for four objectives.'));
end
%GP Model without individual
esin = bestTraining_56(sin_ind);
A = zeros(size(esin));
for i = 1 : n
